...
class Info_Component_Ref:
    ...
    def _resolve_references(self, id_lookup):
        if self.id_ref is not None:
            self.info_component = id_lookup[self.id_ref]
...
class Vehicle_Information:
    ...
    def _build_id_lookup(self, id_lookup):
        if self.vehicle_connectors is not None:
            for vehicle_connector in self.vehicle_connectors:
                vehicle_connector._build_id_lookup(id_lookup)

        if self.physical_vehicle_links is not None:
            for physical_vehicle_link in self.physical_vehicle_links:
                id_lookup[physical_vehicle_link.id] = physical_vehicle_link

    def _resolve_references(self, id_lookup):
        if self.info_component_refs is not None:
            for info_component_ref in self.info_component_refs:
                info_component_ref._resolve_references(id_lookup)

        if self.physical_vehicle_links is not None:
            for physical_vehicle_link in self.physical_vehicle_links:
                physical_vehicle_link._resolve_references(id_lookup)

        if self.logical_links is not None:
            for logical_link in self.logical_links:
                logical_link._resolve_references(id_lookup)
