...
def read_scale_constr_from_odx(et_element):
    if et_element is None:
        return None
    validity = et_element.get("VALIDITY") if et_element.find("VALIDITY") is not None else None
    description = read_description_from_odx(et_element.find("DESC"))
    short_label = et_element.find("SHORT-LABEL").text if et_element.find("SHORT-LABEL") is not None else None
...
class Internal_Constr:
    def __init__(self,
                 lower_limit,
                 upper_limit,
                 scale_constrs: list[Scale_Constr]):
        self.lower_limit = lower_limit
        self.upper_limit = upper_limit
        self.scale_constrs = scale_constrs
...
def read_internal_constr_from_odx(et_element):
    ...
    lower_limit = et_element.find("LOWER-LIMIT").text if et_element.find("LOWER-LIMIT") is not None else None
    upper_limit = et_element.find("UPPER-LIMIT").text if et_element.find("UPPER-LIMIT") is not None else None
    scale_constrs = [read_scale_constr_from_odx(el)
                     for el in et_element.iterfind("SCALE-CONSTRS/SCALE-CONSTR")]
