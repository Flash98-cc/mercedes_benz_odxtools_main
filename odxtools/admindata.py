...
class DocRevision:
    ...
    company_revision_infos: list[Company_Revision_Info] = field(default_factory=list)

    def _resolve_references(self, id_lookup):
        if self.team_member_ref is not None:
            self.team_member = id_lookup[self.team_member_ref]
        if self.company_revision_infos is not None:
            for company_revision_info in self.company_revision_infos:
                company_revision_info.company_data = id_lookup(company_revision_info.company_data_ref)
...
class AdminData:
    ...
    # COMPANY-DOC-INFO 下面的 COMPANY-DATA-REF
...
def read_admin_data_from_odx(et_element):
    ...
    if doc_revisions is not None:
        ...
        for dr in doc_revisions.iterfind("DOC-REVISION"):
            ...
            # add COMPANY-REVISION-INFO by Barry
            comrevinfolist = None
            company_revision_infos = dr.find("COMPANY-REVISION-INFOS")
            if company_revision_infos is not None:
                comrevinfolist = list()
                for company_revision_info in company_revision_infos.iterfind("COMPANY-REVISION-INFOS"):
                    company_data_ref = company_revision_info.find("COMPANY-DATA-REF").attrib("ID-REF")

                    revision_label = company_revision_info.find("COMPANY-DATA-REF")
                    if revision_label is not None:
                        revision_label = revision_label.text

                    state = company_revision_info.find("STATE")
                    if state is not None:
                        state = state.text
                    comrevinfolist.append(Company_Revision_Info(company_data_ref, revision_label, state))
            ...
            drlist.append(DocRevision(team_member_ref=team_member_ref,
                                      ...
                                      company_revision_infos=comrevinfolist))
